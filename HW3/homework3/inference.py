...
def constructBayesNet(gameState: hunters.GameState):
    ...
    "*** YOUR CODE HERE ***"
    variableDomainsDict[PAC]=list()
    variableDomainsDict[GHOST0] =list()
    variableDomainsDict[GHOST1] =list()
    variableDomainsDict[OBS0] = list()
    variableDomainsDict[OBS1] = list()
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        currentFactorsList = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        for joinv in eliminationOrder:
            currentFactorsList, joinedFactor = joinFactorsByVariable(currentFactorsList, joinv)
            #print(currentFactorsList)
            #print(joinedFactor)
            if len(joinedFactor.unconditionedVariables())<=1:
                continue
            currentFactorsList.append(eliminate(joinedFactor, joinv))
